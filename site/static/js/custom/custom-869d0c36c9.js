...
function successAnimation() {
	$('.contact-form > *:not(.svgloader)').css("opacity", "0");
	setTimeout(function () {
		$('.svgloader').removeClass("display-none");
		$('.svgloader').css("opacity", "1");
	}, 300);
	setTimeout(function () {
		$('.svgloader').css("opacity", "0");
		$('.contact-form > *:not(.svgloader)').css("opacity", "1");
		$('.svgloader').addClass("display-none");
	}, 2600);
}
$(".netlifyForm").submit(function (e) {
	e.preventDefault();
	var form = $(this);
	...
	jQuery.post(form.attr('action'), form.serialize()).then(function () {
		successAnimation();
		jQuery(".netlifyForm :input").each(function () {
			if (jQuery(this).attr('name') === 'form-name') { return; }
			jQuery(this).val('');
		});
	});
});
